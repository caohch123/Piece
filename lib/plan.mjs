// 规划：把句子变成分镜（选模板 + 填内容）。有模型配置时用 LLM，否则用启发式。
...
async function llmScenes(units, log, llm) {
  if (!llm) return null;
...
要求：ids 连续覆盖所有句子且不重叠；hero 必须有 title 和 big(核心观点,<=16字)；outro 必须有 title、ok、no、tags(<=3个,每个<=6字)；compare 必须给 left/right；其他模板给 items。文案要口语化、短、精确。`;
  try {
    const content = await chatCompletion(llm, [
      { role: "system", content: sys },
      { role: "user", content: `口播稿（共 ${units.length} 句）：\n${list}` }
    ]);
    const plan = JSON.parse(content.replace(/^```(?:json)?\s*|\s*```$/gi, ""));
    if (!Array.isArray(plan.scenes) || !plan.scenes.length) throw new Error("分镜格式错误");
    const ids = plan.scenes.flatMap((scene) => scene.ids || []);
    if (ids.length !== units.length || ids.some((id, i) => !Number.isInteger(id) || id !== i)) {
      throw new Error("分镜未连续覆盖全部句子");
    }
    const scenes = [];
    plan.scenes.forEach((sc, idx) => {
      ...
      const type = idx === 0 ? "hero" : idx === plan.scenes.length - 1 ? "outro" :
        (["keypoints", "steps", "compare", "diagram", "chart"].includes(sc.type) ? sc.type : "keypoints");
    ...
    log(`${llm.model} 分镜：${scenes.length} 个场景（${scenes.map((s) => s.type).join(" → ")}）`);
    return scenes;
  } catch (e) {
...
async function scriptUnits(text, log, llm) {
  if (llm) {
    try {
      const body = await chatCompletion(llm, [
      ...
      if (lines.length >= 3) { log(`${llm.model} 写稿：${lines.length} 句`); return lines; }
      log("模型写稿句数不足，改用原始提示词切句");
...
export async function plan({ sentences, mode, prompt, log, llm }) {
  let units;
  if (mode === "prompt") {
    const lines = await scriptUnits(prompt, log, llm);
    let t = 0.4;
  ...
  const scenes = (await llmScenes(units, log, llm)) || heuristicScenes(units, log);
