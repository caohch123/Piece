...
export async function execute(job, log, setProgress) {
  ...
  const { total } = build({ workDir, plan, caps, videoFile, log });
  setProgress(55);

  job.stage = "校验";
  await check(workDir, log);
  setProgress(60);

  job.stage = "渲染";
  const out = await render(workDir, "final.mp4", log, (p) => setProgress(60 + Math.round(p * 0.39)));
