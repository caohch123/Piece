...
function title(t) {
  return t ? `<div class="g-title">${esc(t)}</div>` : "";
}
...
export const BUILDERS = {
  hero(sc) {
    return {
      html: `${kicker(sc.kicker, 128)}<div class="ov-title">${esc(sc.title || "")}</div><div class="ov-big">${esc(sc.big || "")}</div>`,
      reveals: [0.3, 0.7]
    };
  },
  card(sc) {
    return {
      html: `${kicker(sc.kicker)}${title(sc.title)}<div class="content"><div class="vcard">${esc(sc.text || "")}${sc.sub ? `<div class="sub">${esc(sc.sub)}</div>` : ""}<div class="sweep"></div></div></div>`,
      reveals: [0.7]
    };
  },
  keypoints(sc) {
  ...
  outro(sc) {
    ...
    return {
      html: `${kicker(sc.kicker, 128)}<div class="ov-title">${esc(sc.title || "")}</div><div class="verdicts">` +
